...
const api = { 
  ...
  readPdfFile: function (filePath: string) { return ipcRenderer.invoke('statements:read-pdf', filePath); },
  ...
  searchGoogleTransaction: function (query: string) { return ipcRenderer.invoke('search:google-transaction', query); },
  createCategory: function (payload: { name: string; group_name: string }) { return ipcRenderer.invoke('categories:create', payload); },
  exportLivingExpense: function (payload: { projectId: string; exportType: string; exportMonth: string | null; exportRangeStart: string | null; exportRangeEnd: string | null; versionLabel: string | null }) { return ipcRenderer.invoke('projects:export-living-expense', payload); }, 
