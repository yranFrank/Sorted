...
export function registerIpc(mainWindow: BrowserWindow): void { 
  ...
  ipcMain.handle('statements:read-pdf', async function (_event, filePath: string) { const buffer = await readFile(filePath); return new Uint8Array(buffer); });
  ...
  ipcMain.handle('search:google-transaction', async function (_event, query: string) { const url = 'https://www.google.com/search?q=' + encodeURIComponent(query); await shell.openExternal(url); return { ok: true, url: url }; });
  ...
  ipcMain.handle('projects:export-living-expense', function (_event, payload: { projectId: string; exportType: string; exportMonth: string | null; exportRangeStart: string | null; exportRangeEnd: string | null; versionLabel: string | null }) { const data = getProjectExportData(payload.projectId); const result = exportLivingExpenseWorkbook({ ...data, exportType: payload.exportType, exportMonth: payload.exportMonth, exportRangeStart: payload.exportRangeStart, exportRangeEnd: payload.exportRangeEnd, versionLabel: payload.versionLabel }); recordExportHistory({ project_id: payload.projectId, export_type: payload.exportType, export_month: payload.exportType === 'custom' && payload.exportRangeStart && payload.exportRangeEnd ? payload.exportRangeStart + '..' + payload.exportRangeEnd : payload.exportMonth, file_path: result.filePath, version_label: payload.versionLabel }); return result; }); 
